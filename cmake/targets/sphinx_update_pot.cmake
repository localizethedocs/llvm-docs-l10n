...
if (VERSION VERSION_LESS_EQUAL    "9" AND
    ...
    message(STATUS "Patching 'html_sidebars' dictionary in 'conf.py' file...")
    set(SPHINX_CONF_PY_FILE "${PROJ_OUT_REPO_DOCS_CONFIG_DIR}/conf.py")
    file(READ "${SPHINX_CONF_PY_FILE}" SPHINX_CONF_PY_CNT)
    set(HTML_SIDEBARS_REGEX "(html_sidebars[ ]*=[ ]*)([^\\n]*[^\\{][\\n\\r\\s]*\\{[^}]*\\})")
    string(REGEX MATCH "${HTML_SIDEBARS_REGEX}" OLD_HTML_SIDEBARS "${SPHINX_CONF_PY_CNT}")
    if (OLD_HTML_SIDEBARS)
        set(NEW_HTML_SIDEBARS "html_sidebars = {'index': ['indexsidebar.html']}")
        string(REGEX REPLACE "${HTML_SIDEBARS_REGEX}" "${NEW_HTML_SIDEBARS}" NEW_SPHINX_CONF_PY_CNT "${SPHINX_CONF_PY_CNT}")
        file(WRITE "${SPHINX_CONF_PY_FILE}" "${NEW_SPHINX_CONF_PY_CNT}")
        remove_cmake_message_indent()
        message("")
        message("Patched 'html_sidebars' dictionary in 'conf.py' file.")
        message("File: ${SPHINX_CONF_PY_FILE}")
        message("")
        message("[OLD_HTML_SIDEBARS_BEGIN]")
        message("${OLD_HTML_SIDEBARS}")
        message("[OLD_HTML_SIDEBARS_END]")
        message("[NEW_HTML_SIDEBARS_BEGIN]")
        message("${NEW_HTML_SIDEBARS}")
        message("[NEW_HTML_SIDEBARS_END]")
        message("")
        restore_cmake_message_indent()
    else()
        remove_cmake_message_indent()
        message("")
        message("'html_sidebars' not found in 'conf.py'. No need to patch.")
        message("File: ${SPHINX_CONF_PY_FILE}")
        message("")
        restore_cmake_message_indent()
    endif()
...
message(STATUS "Patching 'indexsidebar.html' file to the sphinx templates directory...")
file(MAKE_DIRECTORY "${PROJ_OUT_REPO_DOCS_TMPLS_DIR}")
file(COPY_FILE
    "${PROJ_CMAKE_CUSTOM_DIR}/patch/indexsidebar.html"
    "${PROJ_OUT_REPO_DOCS_TMPLS_DIR}/indexsidebar.html")
remove_cmake_message_indent()
message("")
message("From: ${PROJ_CMAKE_CUSTOM_DIR}/patch/indexsidebar.html")
message("To:   ${PROJ_OUT_REPO_DOCS_TMPLS_DIR}/indexsidebar.html")
message("")
restore_cmake_message_indent()


message(STATUS "Patching 'llvm-theme.css' file to the '_themes/llvm-theme/static' directory...")
file(MAKE_DIRECTORY "${PROJ_OUT_REPO_DOCS_DIR}/_themes/llvm-theme/static")
file(COPY_FILE
    "${PROJ_CMAKE_CUSTOM_DIR}/patch/llvm-theme.css"
    "${PROJ_OUT_REPO_DOCS_DIR}/_themes/llvm-theme/static/llvm-theme.css")
remove_cmake_message_indent()
message("")
message("From:  ${PROJ_CMAKE_CUSTOM_DIR}/patch/llvm-theme.css")
message("To:    ${PROJ_OUT_REPO_DOCS_DIR}/_themes/llvm-theme/static/llvm-theme.css")
message("")
restore_cmake_message_indent()
